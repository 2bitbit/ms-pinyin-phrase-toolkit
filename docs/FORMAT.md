...
直接写文件后打字不生效。两条已验证的重载路径：

1. 杀 `TextInputHost` + `ChsIME`（候选宿主才缓存 EUDP），再 Win+Space 切走切回。
2. 设置页「导入」/「添加」（`ms-settings:regionlanguage-chsime-pinyin-udp`）。

只杀 `ChsIME` 不够：已学过的词可能碰巧生效（它们在 `ChsPinyinIH.dat` 里），
新触发码不会。单独 Win+Space 也不够。

### 4. 记录必须按拼音排序

官方导出与 GUI 导入产物均为 `(拼音, 候选位置)` 序。无序时 IME 按有序表查找会错位。
