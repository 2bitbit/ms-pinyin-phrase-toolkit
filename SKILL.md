...
输入法启动时读 `ChsPinyinEUDPv1.lex` 并缓存。判据是**打字能否打出新短语**
（设置页列表刷新只说明页面读到了文件）。
...
| 直接改 `.lex` 文件 | ❌ |
| 只按 Win+Space | ❌ |
| 只杀 `ChsIME` | 部分：已学过的词（进了 `ChsPinyinIH.dat`）可能碰巧生效 |
...
1. **格式是否正确** —— 跑 `roundtrip`。对 GUI「导出」的 `.dat` 应为 `逐字节一致: True`。
2. **是否完成重载** —— 无 GUI 路径必须杀 `TextInputHost`+`ChsIME` 再 Win+Space；
   只切输入法或只杀 `ChsIME` 不够。
3. **写出是否按拼音排序** —— 无序时查找错位（打 `p` 带出 `t` 的词，打 `t` 什么都没有）。
4. **触发码是否与真实拼音抢位** —— 双字母干扰小；单字母会和常用字抢候选。
